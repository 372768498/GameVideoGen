...
export default function GameVideoGenPage() {
  ...
  const handlePasswordSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const correctPassword = 'gamevideo2025';
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const videoResponse = await fetch('/api/game-video-gen/generate-video', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ script, formData }),
      });
      if (!videoResponse.ok) {
          ...
          <div className="bg-gray-800 rounded-lg shadow-2xl p-8 border border-gray-700">
            ...
            <form onSubmit={handlePasswordSubmit}>
              <div className="space-y-4">
                ...
                <Button type="submit" className="w-full bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 text-white">验证</Button>
              </div>
            </form>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 py-12 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-5xl font-bold text-white mb-4">🎮 AI游戏视频生成器</h1>
          <p className="text-xl text-gray-300">使用AI自动生成专业游戏宣传视频</p>
          <p className="text-sm text-gray-400 mt-2">Powered by OpenAI GPT-4 & FAL.AI SORA2</p>
        </div>
        <div className="bg-gray-800 rounded-lg shadow-2xl p-8 border border-gray-700">
          <div className="space-y-6">
            <div>
              <Label htmlFor="gameTitle" className="text-white">游戏名称 *</Label>
              <Input id="gameTitle" value={formData.gameTitle} onChange={(e) => setFormData({ ...formData, gameTitle: e.target.value })} placeholder="例如：狱国争霸" className="mt-2 bg-gray-700 border-gray-600 text-white" />
              {errors.gameTitle && <p className="text-red-400 text-sm mt-1">{errors.gameTitle}</p>}
            </div>
            <div>
              <Label htmlFor="gameDescription" className="text-white">游戏介绍 * (50-1000字符)</Label>
              <Textarea id="gameDescription" value={formData.gameDescription} onChange={(e) => setFormData({ ...formData, gameDescription: e.target.value })} placeholder="详细描述游戏特点、玩法、风格等..." rows={6} className="mt-2 bg-gray-700 border-gray-600 text-white" />
              <p className="text-gray-400 text-sm mt-1">{formData.gameDescription.length} / 1000 字符</p>
              {errors.gameDescription && <p className="text-red-400 text-sm mt-1">{errors.gameDescription}</p>}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Select value={formData.language} onValueChange={(value: any) => setFormData({ ...formData, language: value })}>
                  <SelectTrigger className="mt-2 bg-gray-700 border-gray-600 text-white"><SelectValue /></SelectTrigger>
                  <SelectContent><SelectItem value="zh-CN">中文</SelectItem><SelectItem value="en-US">English</SelectItem></SelectContent>
                </Select>
              </div>
              <div>
                ...
                <Select value={formData.platform} onValueChange={handlePlatformChange}>
                  <SelectTrigger className="mt-2 bg-gray-700 border-gray-600 text-white"><SelectValue /></SelectTrigger>
                  <SelectContent><SelectItem value="douyin">抖音</SelectItem><SelectItem value="kuaishou">快手</SelectItem><SelectItem value="youtube">YouTube</SelectItem></SelectContent>
                </Select>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Select value={formData.duration.toString()} onValueChange={(value) => setFormData({ ...formData, duration: parseInt(value) as 4 | 8 | 12 })}>
                  <SelectTrigger className="mt-2 bg-gray-700 border-gray-600 text-white"><SelectValue /></SelectTrigger>
                  <SelectContent><SelectItem value="4">4秒 (2个场景)</SelectItem><SelectItem value="8">8秒 (4个场景)</SelectItem><SelectItem value="12">12秒 (6个场景)</SelectItem></SelectContent>
                </Select>
              </div>
              <div>
                ...
                <Select value={formData.aspectRatio} onValueChange={(value: any) => setFormData({ ...formData, aspectRatio: value })}>
                  <SelectTrigger className="mt-2 bg-gray-700 border-gray-600 text-white"><SelectValue /></SelectTrigger>
                  <SelectContent><SelectItem value="9:16">9:16 竖屏</SelectItem><SelectItem value="16:9">16:9 横屏</SelectItem></SelectContent>
                </Select>
              </div>
            </div>
            <div className="bg-blue-900/20 border border-blue-500/30 rounded-lg p-4">
              <p className="text-blue-300 text-sm text-center">💰 预估成本: ${getCostEstimate()} USD</p>
            </div>
            <Button onClick={handleGenerate} className="w-full bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 text-white text-lg py-6">🎬 生成视频</Button>
          </div>
        </div>
